...
if ($token_id) {
    ...
    $ftoken_id = '#T-' . htmlspecialchars((string)$token_id);
}
...
<?php if ($token_id !== null):?>
    <p>Your token has been created. Please note your token number: <b><?php echo  $ftoken_id; ?></b></p>
    <br>
    ...
    <?php if (!empty($tokens_before)):?>
        <table class="app-table">
            ...
            <?php foreach ($tokens_before as $token):?>
                <tr>
                    <td><?php echo '#T-' . htmlspecialchars((string)$token['token_id']); ?></td>
                    <td><?php echo htmlspecialchars($token['fullname']); ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
    <?php else:?>
...
<form action="../controller/token-handler.php" method="POST">
    <input type="submit" <?php $token_exists = $token_model->token_already_exists($_SESSION['id']);
